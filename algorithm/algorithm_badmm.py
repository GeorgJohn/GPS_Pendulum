...
from algorithm.utils import PolicyInfo
from algorithm.config import ALG_BADMM
...
class AlgorithmBADMM(Algorithm):
    ...
    def _set_interp_values(self):
        ...
        if type(self._hyperparams['ent_reg_schedule']) in (int, float):
            ...
            self.policy_opt.set_ent_reg(np.exp(np.interp(t, np.linspace(0, 1, num=len(sch)), np.log(sch))))
        ...
        if type(self._hyperparams['lg_step_schedule']) in (int, float):
            ...
            self._hyperparams['lg_step'] = np.exp(np.interp(t, np.linspace(0, 1, num=len(sch)), np.log(sch)))
            ...
            for t in range(T):
                ...
                for i in range(N):
                    mu[i, t, :] = (traj.K[t, :, :].dot(X[i, t, :]) + traj.k[t, :]) - \
                                  np.linalg.solve(prc[i, t, :, :] / pol_info.pol_wt[t],
    ...
    def _policy_dual_step(self, m, step=False):
        ...
        for t in range(T):
            ...
            # Increment mean term.
            pol_info.lambda_k[t, :] -= \
                self._hyperparams['policy_dual_rate'] * \
                pol_info.pol_wt[t] * \
    ...
    def _stepadjust(self, m):
        ...
        # This is the policy that the previous samples were actually
        # drawn from under the dynamics that were estimated from the
        # previous samples.
    ...
    def _policy_kl(self, m, prev=False):
        ...
        for t in range(T):
            ...
            kl[:, t] = 0.5 * np.sum(np.sum(tr_pp_ct, axis=1), axis=1) - k_ln_det_ct + 0.5 * ln_det_cp + 0.5 * d_pp_d
            tr_pp_ct_m = np.mean(tr_pp_ct, axis=0)
            kl_m[t] = 0.5 * np.sum(np.sum(tr_pp_ct_m, axis=0), axis=0) - k_ln_det_ct + 0.5 * np.mean(ln_det_cp) + 0.5 * np.mean(d_pp_d)
            ...
            kl_l[:, t] = 0.5 * np.sum(np.sum(tr_pp_ct, axis=1), axis=1) - k_ln_det_ct + 0.5 * ln_det_cp + 0.5 * d_pp_d_l
            kl_lm[t] = 0.5 * np.sum(np.sum(tr_pp_ct_m, axis=0), axis=0) - k_ln_det_ct + 0.5 * np.mean(ln_det_cp) + 0.5 * np.mean(d_pp_d_l)
