...
from algorithm.policy_opt.config import POLICY_OPT_TF
import tensorflow as tf

from algorithm.policy.tf_policy import TfPolicy
from algorithm.policy_opt.policy_opt import PolicyOpt
from algorithm.policy_opt.tf_utils import TfSolver
...
class PolicyOptTf(PolicyOpt):
    ...
    def init_network(self):
        ...
        tf_map, fc_vars, last_conv_vars = tf_map_generator(
            dim_input=self._dO,
            dim_output=self._dU,
    ...
    def update(self, obs, tgt_mu, tgt_prc, tgt_wt):
        ...
        for i in range(self._hyperparams['iterations']):
            ...
            if (i+1) % 50 == 0:
                ...
                tmp_loss_history.append(average_loss/50)
                average_loss = 0

        self.train_loss_history.append(tmp_loss_history)
        DataLogger().pickle(
            self._hyperparams['data_files_dir'] + 'policy_train_loss_history.pkl',
            self.train_loss_history
        )
