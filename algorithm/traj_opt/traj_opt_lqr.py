...
from algorithm.traj_opt.config import TRAJ_OPT_LQR
from algorithm.traj_opt.traj_opt import TrajOpt
from algorithm.traj_opt.utils import \
...
class TrajOptLQR(TrajOpt):
    ...
    def update(self, m, algorithm):
        ...
        for itr in range(max_itr):
            ...
            if self._conv_check(con, kl_step):
                if not self.cons_per_step:
                    LOGGER.debug("KL: %f / %f, converged iteration %d",
                                 kl_div, kl_step, itr)
                else:
                    LOGGER.debug("KL: %f / %f, converged iteration %d",
            ...
            if not self.cons_per_step:
                ...
                if itr % 10 == 0:
                    LOGGER.debug("avg KL: %f / %f, avg new eta: %f",
                                 np.mean(kl_div[:-1]), np.mean(kl_step[:-1]), np.mean(eta[:-1]))
    ...
    def backward(self, prev_traj_distr, traj_info, eta, algorithm, m):
        ...
        while fail:
            ...
            fCm, fcv = algorithm.compute_costs(m, eta, augment=(not self.cons_per_step))
            ...
            for t in range(T - 1, -1, -1):
                ...
                if not self.cons_per_step:
                    ...
                    K_term = (1.0 / eta[t]) * Qtt[t, idx_u, idx_x] - prev_traj_distr.inv_pol_covar[t].dot(prev_traj_distr.K[t])
                ...
                if self.cons_per_step or not self._hyperparams['update_in_bwd_pass']:
