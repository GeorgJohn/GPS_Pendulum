...
class Agent(object):
    ...
    def __init__(self, hyperparams):
        ...
        self._meta_data_idx = {d: i for d, i in zip(self.meta_data_types, self._meta_idx)}

    @abc.abstractmethod
    def sample(self, policy, condition, verbose=True, save=True, noisy=True):
        """
        Draw a sample from the environment, using the specified policy and under the specified condition,
    ...
    def get_samples(self, condition, start=0, end=None):
        ...
        :param condition: Condition for which to get the sample.
    ...
    def clear_samples(self, condition=None):
        ...
        :param condition: Condition for which to reset samples.
        """
    ...
    @staticmethod
    def pack_data(existing_mat, data_to_insert, data_types, dim, idx, axes=None):
        """
        Update matrix with new data.
        :param existing_mat:   Current observation matrix.
        :param data_to_insert: New data to insert into the existing matrix.
        :param data_types:     Name of the sensors to insert data for.
        :param dim:            Data dimension
        :param idx:            indices corresponding to a certain sensor name
        :param axes:           Which axes to insert data. Defaults to the last axes.
        """
        ...
        for i in range(num_sensor):
            ...
            if existing_mat.shape[axes[i]] != dim:
                raise ValueError('Axes must be along an dX=%d dimensional axis', dim)
            insert_shape[axes[i]] = len(idx[data_types[i]])
        if tuple(insert_shape) != data_to_insert.shape:
            raise ValueError(f'Data has shape {data_to_insert.shape}. Expected {tuple(insert_shape)}')

        # Actually perform the slice.
        index = [slice(None) for _ in range(len(existing_mat.shape))]
        for i in range(num_sensor):
            index[axes[i]] = slice(idx[data_types[i]][0], idx[data_types[i]][-1] + 1)
    ...
    def pack_data_obs(self, existing_mat, data_to_insert, data_types, axes=None):
        ...
        :param axes:           Which axes to insert data. Defaults to the last axes.
        """
        self.pack_data(existing_mat, data_to_insert, data_types, self.dO, self._obs_data_idx, axes)

    def pack_data_meta(self, existing_mat, data_to_insert, data_types, axes=None):
        """
        Update the meta data matrix with new data.
        :param existing_mat:   Current meta data matrix.
        :param data_to_insert: New data to insert into the existing matrix.
        :param data_types:     Name of the sensors to insert data for.
        :param axes:           Which axes to insert data. Defaults to the last axes.
        """
        self.pack_data(existing_mat, data_to_insert, data_types, self.dM, self._meta_data_idx, axes)

    def pack_data_x(self, existing_mat, data_to_insert, data_types, axes=None):
        """
        Update the state matrix with new data.
        :param existing_mat:   Current state matrix.
        :param data_to_insert: New data to insert into the existing matrix.
        :param data_types:     Name of the sensors to insert data for.
        :param axes:           Which axes to insert data. Defaults to the last axes.
        """
        self.pack_data(existing_mat, data_to_insert, data_types, self.dX, self._x_data_idx, axes)

    def unpack_data_x(self, existing_mat, data_types, axes=None):
        """
        Returns the requested data from the state matrix.
        :param existing_mat: State matrix to unpack from.
        :param data_types:   Names of the sensor to unpack.
        :param axes:         Which axes to unpack along. Defaults to the last axes.
        """
        num_sensor = len(data_types)
        if axes is None:
            # If axes not specified, assume indexing on last dimensions.
